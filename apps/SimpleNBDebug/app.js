...
SimpleNBDebug.prototype.update = function(streams){
    ...
    str += '<tr> <td style="text-align:center;"> Deformed: ' + streams["stats"].beams_deformed + " ("+((streams["stats"].beams_deformed/streams["stats"].beam_count)*100).toFixed(2) + '%) </td> <td style="text-align:center;"> Total Weight: ' + streams["stats"].total_weight.toFixed(2) + 'kg Chassis Weight: ' + (streams["stats"].total_weight - streams["stats"].wheel_weight).toFixed(2) + 'kg </td> </tr>';
    
    str += '<tr> <td style="text-align:center;"> Broken: ' + streams["stats"].beams_broken + " ("+((streams["stats"].beams_broken/streams["stats"].beam_count)*100).toFixed(2) + '%)</td> <td style="text-align:center;">' + (streams["stats"].wheel_weight/streams["stats"].wheel_count).toFixed(2) + 'kg per Wheel';
